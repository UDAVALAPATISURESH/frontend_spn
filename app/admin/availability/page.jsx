...
export default function AvailabilityPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      ...
      if (typeof window !== 'undefined') {
        ...
        const staffIdParam = params.get('staffId');
        if (staffIdParam) {
          setSelectedStaff(parseInt(staffIdParam));
        }
      }
    }
  }, [authLoading]);
  ...
  return (
    <div>
      ...
      <div className="list">
        {availability.length === 0 ? (
          <div className="card">No schedules found.</div>
        ) : (
          availability.map((schedule) => {
            const dayLabel =
              DAYS.find((d) => d.value === schedule.dayOfWeek)?.label || 'Unknown';

            return (
              <div key={schedule.id} className="card">
                <h3>{dayLabel}</h3>
                <p>{schedule.startTime} - {schedule.endTime}</p>
                <button className="btn btn-secondary" onClick={() => handleEdit(schedule)}>Edit</button>
                <button className="btn btn-secondary" onClick={() => handleDelete(schedule.id)}>Delete</button>
              </div>
            );
          })
        )}
